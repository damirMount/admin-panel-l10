...
@section('content')
    <div class="col-auto d-flex justify-content-end">
        <a href="{{ route('admin_businesses.create') }}" class="btn btn-success">{{ __('Создать') }}</a>
    </div>
    <div class="m-2 rounded">
        <table class="table table-striped table-bordered rounded" id="businesses-table">
            <thead>
            <tr>
                <th></th>
...
@push('scripts')
    ...
    <script>
        $(function () {
            let table = $('#businesses-table').DataTable({
                responsive: true,
                processing: true,
                ajax: '{!! route('admin_businesses.index') !!}',
                columns: [
            ...
            $('#businesses-table').on('click', 'tbody td.control', function () {
            ...
            $('#businesses-table tbody').on('click', 'td:not(.control)', function () {
                let data = table.row(this.parentElement).data();
                window.location.href = window.location.origin + '/admin/businesses/' + data.id+'';
            });
        });
    </script>
@endpush
