...
@section('content')
    <div class="col-auto d-flex justify-content-end">
        <a href="{{ route('admin_families.create') }}" class="btn btn-success">{{ __('Создать') }}</a>
    </div>
    <div class="m-2 rounded">
        <table class="table table-striped table-bordered rounded" id="families-table">
            <thead>
            <tr>
                <th></th>
...
@push('scripts')
    ...
    <script>
        $(function () {
            let table = $('#families-table').DataTable({
                responsive: true,
                processing: true,
            ...
            $('#families-table').on('click', 'tbody td.control', function () {
            ...
            $('#families-table tbody').on('click', 'td:not(.control)', function () {
                let data = table.row(this.parentElement).data();
                window.location.href = window.location.origin + '/admin/families/' + data.id+'';
            });
        });
    </script>
@endpush
